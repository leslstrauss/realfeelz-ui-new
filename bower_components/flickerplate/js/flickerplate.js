	...
	Plugin.prototype = {
		init: function() {
			...
			function fc_hammer_flick() {
				...
				if ($settings.flick_animation == 'transform-slide' || $settings.flick_animation == 'transition-slide' || $settings.flick_animation == 'jquery-slide') {
					// Interaction
					$flicker.find('ul.flicks').hammer()
						.on('panleft panright', function($ev) {
							fc_interaction_pan($ev)
						})
						.on('panend', function($ev) {
							fc_interaction_panend($ev)
						});
				} else if ($settings.flick_animation == 'transition-fade' || $settings.flick_animation == 'jquery-fade') {
					$flicker.find('ul.flicks').hammer()
						.on('swipeleft swiperight', function($ev) {
							fc_interaction_swipe($ev)
						});
				}
			};
			...
			function fc_move_flicker() {
				...
				switch ($settings.flick_animation) {
					...
					case 'jquery-slide':
						...
						$flicker.find('ul.flicks').animate({
							'left': '-' + $position + '00%'
						}, function() {
							$flicker_moving = false;
						});
